...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            ...
            if (_crashOnErrors) {
                //NSAssert1(false, @"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                abort();
            }
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    int rc;
    char *errmsg = nil;
    callback_type t =  block ? (callback_type)FMDBExecuteBulkSQLCallback : (callback_type)nil;
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String],t
